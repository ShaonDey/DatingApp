...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        {
            return await _context.Likes.FirstOrDefaultAsync(u => u.LikerId.Equals(userId) && u.LikeeId.Equals(recipientId));
        }
        ...
        {
            var users = _context.Users.Include(a => a.Photos).OrderByDescending(u => u.LastActive).AsQueryable();
        ...
        {
            var user = await _context.Users.Include(x => x.Likers).Include(x => x.Likees).FirstOrDefaultAsync(u => u.Id.Equals(id));
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams)
        {
            ...
            {
                case "Inbox":
                    messages = messages.Where(u => u.RecipientId.Equals(messageParams.UserId) && !u.RecipientDeleted);
                    break;
                case "Outbox":
                    messages = messages.Where(u => u.SenderId.Equals(messageParams.UserId) && !u.SenderDeleted);
                    break;
                default:
                    messages = messages.Where(u => u.RecipientId.Equals(messageParams.UserId)
                        && !u.RecipientDeleted && u.IsRead.Equals(false));
                    break;
            }
